	...
	New()
		...
		if (derelict_mode)
			...
			for (var/A in typesof(/datum/job/command))
				if(!(A in map_settings.jobs_exclude))
					src.staple_jobs += new A(src)
			for (var/A in typesof(/datum/job/security))
				if(!(A in map_settings.jobs_exclude))
					src.staple_jobs += new A(src)
			for (var/A in typesof(/datum/job/research))
				if(!(A in map_settings.jobs_exclude))
					src.staple_jobs += new A(src)
			for (var/A in typesof(/datum/job/engineering))
				if(!(A in map_settings.jobs_exclude))
					src.staple_jobs += new A(src)
			for (var/A in typesof(/datum/job/civilian))
				if(!(A in map_settings.jobs_exclude))
					src.staple_jobs += new A(src)
			for (var/A in typesof(/datum/job/special))
				if(!(A in map_settings.jobs_exclude))
